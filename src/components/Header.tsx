...
export function Header() {
  ...
  const iconSize = 20;
  const menuItems = [
    { text: 'Home', variant: 'light', icon: <IconHome width={iconSize} height={iconSize}/>},
    { text: 'Blog', variant: 'subtle', icon: <IconArticle width={iconSize} height={iconSize}/>},
    { text: 'Suporte', variant: 'subtle', icon: <IconBrandWechat width={iconSize} height={iconSize}/>},
  ]

  return (
    <Grid mt={2} justify="space-around" align="center">
      <Grid.Col span={3} style={{border: '0px solid #000'}}>
        <Flex align="center" justify="center">
          ...
          <Title order={matches ? 5 : 4} color="#868e96">ToDo List</Title>
        </Flex>
      </Grid.Col>
      <Grid.Col span={6} style={{border: '0px solid #000'}}>
        <Flex align="center" gap={{ base: 'sm' }} justify="center">
          {menuItems.map((menu) => (
            <ButtonHeader key={menu.text} text={menu.text} variant={menu.variant} icon={menu.icon} matches={matches}/>
          ))}
        </Flex>
      </Grid.Col>
